  ...
  template: `
    <div class="flex h-screen w-full bg-[#f8faf9] text-[#191c1c] font-sans overflow-hidden selection:bg-[#fe893e]/20 selection:text-[#9b4600]">
      ...
      @if (showExportPdfModal()) {
        <div class="fixed inset-0 z-50 flex items-center justify-center bg-[#191c1c]/40 backdrop-blur-sm">
          <div class="bg-white w-full max-w-md p-6 rounded-2xl shadow-xl border border-[#dde3eb]">
            <h2 class="text-lg font-serif font-bold text-[#191c1c] mb-4">Export Registry as PDF</h2>
            
            <div class="space-y-4 mb-6">
              <div>
                <label class="block text-xs font-bold text-[#6e7978] mb-1">From Date</label>
                <input type="date" [(ngModel)]="exportDateFrom" class="w-full bg-[#f2f4f3] px-3 py-2 rounded-xl text-xs text-[#191c1c] border-none focus:outline-none focus:bg-white focus:ring-1 focus:ring-[#004343]" />
              </div>
              <div>
                <label class="block text-xs font-bold text-[#6e7978] mb-1">To Date</label>
                <input type="date" [(ngModel)]="exportDateTo" class="w-full bg-[#f2f4f3] px-3 py-2 rounded-xl text-xs text-[#191c1c] border-none focus:outline-none focus:bg-white focus:ring-1 focus:ring-[#004343]" />
              </div>
            </div>

            <div class="flex justify-end gap-3">
              <button (click)="showExportPdfModal.set(false)" class="px-4 py-2 rounded-xl text-xs font-bold text-[#3f4948] hover:bg-[#f2f4f3] transition-colors">
                Cancel
              </button>
              <button (click)="downloadPdf()" class="px-4 py-2 rounded-xl bg-[#004343] text-white text-xs font-bold hover:bg-[#003131] transition-all shadow-sm">
                Download PDF
              </button>
            </div>
          </div>
...
export class ProfileManagementComponent {
  ...
  downloadPdf(): void {
    ...
    doc.setFontSize(10);
    doc.text('Generated by Admin', 14, 30);
    
    if (this.exportDateFrom && this.exportDateTo) {
      doc.text(`Selected Range: ${this.exportDateFrom} to ${this.exportDateTo}`, 14, 36);
    }
    ...
    autoTable(doc, {
      startY: 45,
      head: [['Identity Code', 'Name', 'Role', 'Heritage Specialty', 'District Hub', 'Status']],
      body: tableData,
    });

    doc.save('community-roster.pdf');
